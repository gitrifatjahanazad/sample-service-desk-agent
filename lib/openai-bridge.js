...
export class OpenAIBridge extends EventEmitter {
  ...
  #requestGreeting() {
    // Kick off a Thai greeting so the caller hears something on pickup.
    this.#send({
      type: 'response.create',
      response: {
        instructions: 'ทักทายผู้ใช้สั้น ๆ เป็นภาษาไทยในฐานะเจ้าหน้าที่ Help Desk ของระบบ ERP และถามว่าวันนี้ให้ช่วยเรื่องอะไรค่ะ',
      },
    });
  }
