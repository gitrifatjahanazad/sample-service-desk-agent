...
// SIP-over-NAT: the address advertised in Via/Contact/SDP must be reachable
// from the PBX. In K8s/cloud the pod's bind address is private — prefer an
// explicit SIP_PUBLIC_ADDRESS, then config.publicAddress, then the host IP.
function resolvePublicIp(cfg) {
  const env = process.env.SIP_PUBLIC_ADDRESS;
  if (env && env.trim()) return env.trim();
  if (cfg?.publicAddress && cfg.publicAddress !== 'Auto') return cfg.publicAddress;
  return pickLocalIp();
}
...
function buildRegisterRequest(cfg, { authHeader = null } = {}) {
  ...
  const localIp = resolvePublicIp(cfg);
  ...
  try {
    ...
    try {
      sip.start({
        port: LOCAL_SIP_PORT,
        address: '0.0.0.0',
        publicAddress: resolvePublicIp(cfg),
        udp: true,
        tcp: false,
        tls: false,
        ws: false,
...
async function handleInvite(rq) {
  ...
  const localIp = resolvePublicIp(currentConfig);
  ...
  ok.headers.contact = [{ uri: `sip:${currentConfig.username}@${resolvePublicIp(currentConfig)}:${LOCAL_SIP_PORT}` }];
  ...
  await new Promise((resolve, reject) => {
    try {
      sip.start({
        ...
        tls: false,
        ws: false,
        logger: process.env.SIP_WIRE_LOG === '1' ? {
          send(m, t) { try { console.log(`\n[SIP SEND -> ${t?.address}:${t?.port}]\n${sip.stringify(m)}`); } catch {} },
          recv(m, r) { try { console.log(`\n[SIP RECV <- ${r?.address}:${r?.port}]\n${sip.stringify(m)}`); } catch {} },
          error(err) { console.error('[SIP ERR]', err?.message || err); },
        } : undefined,
      }, onRequest);
      resolve();
    } catch (err) {
