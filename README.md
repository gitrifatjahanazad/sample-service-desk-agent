# Nepali Voice ERP Help-Desk Agent

A minimal browser-based voice agent that speaks and listens in Nepali, built on the OpenAI Realtime API (`gpt-realtime-1.5`) over WebRTC. Ships with a default ERP (Enterprise Resource Planning) support persona that can be edited live in the UI.
...
OpenAI Realtime API
   ...
   - transcription (gpt-4o-mini-transcribe, language: ne)
...
- Nepali input and output, with live user and assistant transcripts in the UI
...
- Default persona: polite Nepali ERP support agent (Accounting, Sales, Purchase, Inventory, HR/Payroll, Manufacturing, CRM, Invoice, VAT, Reports) — refers unknowns to the user's ERP admin / IT support
...
Open `http://localhost:3000` in Chrome or Edge, grant microphone permission, click **जडान गर्नुहोस्**, and start speaking.
...
| Transcription language | `ne` (Nepali) | [public/app.js](public/app.js), [lib/openai-bridge.js](lib/openai-bridge.js) — `audio.input.transcription.language` |
...
| System prompt | ERP-themed Nepali default, editable in UI | [public/app.js](public/app.js) — `DEFAULT_SYSTEM_PROMPT`, or edit at runtime |

Other valid voices for Realtime: `marin` (also realtime-exclusive), `alloy`, `echo`, `shimmer`. Other transcription models: `gpt-4o-transcribe`, `whisper-1`. Switch `noise_reduction` to `near_field` if you use a close-talking headset mic.
...
└── public/
    ├── index.html          # UI (Nepali labels, prompt editor, SIP panel, log)
...
The server can also act as a SIP softphone (like MicroSIP) so that incoming PSTN/PBX calls are auto-answered and bridged to the same Nepali ERP bot. The bridge is server-side and runs in parallel with the browser flow.

- Open `http://localhost:3000`, expand **📞 कल कन्फिग (SIP Account)**, fill in the same fields you'd put in MicroSIP (SIP Server, Username, Domain, Login, Password, Transport=UDP, Register Refresh, Keep-Alive), click **Save**, then **Start**.
...
- The bot's persona for SIP calls is read from `sip-config.json#instructions`. Use the **SIP मा सुरक्षित गर्नुहोस्** button in the System prompt panel to push the current textarea contents to that file.
...
- No tool calling, no RAG, no knowledge base. The agent answers from the system prompt and the model's general knowledge; for unverified specifics it refers users to their ERP administrator or IT/support team.
